...
@section('content')
    <div class="container mx-auto">
        <h1 class="text-4xl font-bold mb-4">Edit Booking</h1>
        <form action="{{ route('bookings.update', $booking->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="destination_id" id="destination_id" class="border p-2 w-full" required>
                    @foreach($destinations as $destination)
                        ...
                        {{ $destination->id == $booking->destination_id ? 'selected' : '' }}>{{ $destination->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                class="border p-2 w-full" value="{{ $booking->total_price }}" required>
                @error('total_price')
                        <div class="text-red-600 mt-2 text-5m">
                            *{{$message}}
                        </div>
            ...
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Update Booking</button>
        </form>
    </div>
@endsection
