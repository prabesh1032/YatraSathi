        ...
        <div class="container mx-auto px-6 lg:px-16">
            ...
            <div class="text-center mb-12">
                <h2 class="text-5xl font-extrabold text-gray-900 mb-4 tracking-tight">Why Choose <span
                        class="text-yellow-500">YatraSathi?</span></h2>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div>
                    <ul class="space-y-6 text-lg text-gray-700">
                        @foreach ([['ri-global-line', 'Explore exclusive travel deals tailored to your needs.'], ['ri-compass-3-line', 'Get personalized itinerary planning for a seamless journey.'], ['ri-customer-service-2-line', '24/7 customer support to assist you every step of the way.'], ['ri-user-star-line', 'Trusted by thousands of happy travelers worldwide.'], ['ri-shield-star-line', 'Reliable travel insurance for a worry-free experience.'], ['ri-flight-takeoff-line', 'Flexible and budget-friendly travel plans.']] as $feature)
                            <li class="flex items-center group">
                                <div
                                    class="bg-indigo-100 p-4 rounded-full shadow-md mr-4 transition-transform duration-300 group-hover:scale-125 group-hover:bg-gray-900">
                                    <i class="{{ $feature[0] }} text-gray-900 text-3xl group-hover:text-white"></i>
                                </div>
                                <span
                                    class="group-hover:text-yellow-500 transition-colors duration-300">{{ $feature[1] }}</span>
                            </li>
                        @endforeach
                    </ul>
                </div>
                ...
                <a href="{{ route('packages') }}" class="relative group">
                    ...
                    <div class="absolute bottom-6 left-6 text-white">
                        <h3 class="font-bold text-3xl mb-1 group-hover:text-yellow-400 transition-colors duration-300">
                            Explore Popular Packages</h3>
                            ...
                            @foreach ($packages as $package)
                                <a href="{{ route('packages.read', ['package' => $package->id]) }}"
                                    ...
                                    <img src="{{ asset('images/' . $package->photopath) }}"
                                        alt="{{ $package->package_name }}" class="w-full h-80 object-cover rounded-lg">
                                    <div
                                        class="absolute bottom-0 left-0 bg-gradient-to-t from-black  to-transparent text-white p-4">
                                        <h3 class="text-3xl text-yellow-500 font-bold">{{ $package->package_name }}</h3>
                                        <p class="text-xl text-green-600 font-bold">Rs. {{ $package->package_price }}</p>
                                    </div>
                                </a>
                            @endforeach
                        </div>
