        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
                ...
                <form id="filterForm" method="GET" action="{{ route('guides.show') }}">
                    <select
                        class="px-4 py-2 text-sm rounded-full border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none bg-white text-gray-700"
                        name="specialization" onchange="this.form.submit()"
                        style="font-family: 'Plus Jakarta Sans', sans-serif;">
                        <option value="">All Specializations</option>
                        @foreach ($packages as $package)
                            <option value="{{ $package->id }}" @if ($package->id == $selectedPackage) selected @endif>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7">
                @forelse($guides as $guide)
                    <div
                        class="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden group">
                        ...
                        <div class="p-5">
                            ...
                            <div class="relative w-24 h-24 mx-auto mb-4">
                                @if ($guide->photopath)
                                    <img src="{{ asset('images/' . $guide->photopath) }}" alt="{{ $guide->name }}"
                                        class="w-full h-full object-cover rounded-full border-4 border-white shadow-md"
                                        loading="lazy">
                                @else
                                    <div
                                        class="w-full h-full rounded-full bg-blue-50 flex items-center justify-center border-4 border-white shadow-md">
                            ...
                            {{-- Specialization --}}
                            <div
                                class="flex items-center justify-center gap-2 bg-gray-50 border border-gray-100 rounded-xl px-4 py-2.5 mb-4">
                                ...
                                <span class="text-xs font-bold text-blue-900"
                                    style="font-family: 'Plus Jakarta Sans', sans-serif;">
                                    {{ $guide->package->package_name ?? 'Not assigned' }}
                                </span>
                            </div>
                ...
                @empty
                    <div class="col-span-full text-center py-20">
                        ...
                        <p class="text-gray-400 text-sm" style="font-family: 'Plus Jakarta Sans', sans-serif;">
