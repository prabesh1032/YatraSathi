...
<body class="bg-gray-100 text-gray-800">
    <div class="max-w-3xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden mt-8">
        ...
        <!-- Main Content -->
        <div class="p-8">
            <p class="text-lg font-semibold text-gray-700 mb-4">Dear ,</p>
            <p class="text-gray-600 leading-relaxed mb-6">
                We are thrilled to inform you that your booking status is now
                <span class="font-bold text-blue-600">{{ $status }}</span>. Thank you for choosing YatraSathi for
                your travel needs!
            </p>
            ...
            <ul class="space-y-3 text-gray-700">
                <li><strong>Package Name:</strong> {{ $package->package_name ?? ($packageName ?? 'Package') }}</li>
                <li><strong>Total Price:</strong> Rs.
                    {{ number_format((float) ($order->total_price ?? ($totalPrice ?? 0)), 2) }}</li>
                <li><strong>Number of Travelers:</strong> {{ $order->num_people ?? ($numPeople ?? 'N/A') }}</li>
                <li><strong>Duration:</strong> {{ $package->duration ?? 'N/A' }} Days</li>
                <li><strong>Traveling On:</strong>
                    {{ \Carbon\Carbon::parse($order->travel_date ?? $travelDate)->format('d M Y') }}</li>
                <li><strong>Payment Method:</strong> {{ $order->payment_method ?? ($paymentMethod ?? 'N/A') }}</li>
            </ul>
            ...
            <p class="text-gray-600 leading-relaxed">
                ...
                <a href="mailto:support@yatrasathi.com"
                    class="text-blue-600 hover:underline">support@yatrasathi.com</a>.
            </p>

            <div class="text-center mt-8">
                <a href="{{ route('home') }}"
                    class="bg-blue-600 text-white px-6 py-3 rounded-md text-lg font-semibold shadow hover:bg-blue-700 transition">
