...
@section('content')
    <div class="bg-gray-100 py-10">
        ...
        <div class="container mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10 px-5 md:px-20">
            <!-- Package Information -->
            <div
                class="col-span-1 bg-gradient-to-r from-white to-gray-100 p-6 rounded-lg shadow-lg transform hover:scale-105 transition-transform duration-300 ease-in-out relative">
                <!-- Package Image -->
                @if ($package->photopath && file_exists(public_path('images/' . $package->photopath)))
                    <img src="{{ asset('images/' . $package->photopath) }}" alt="Package Image"
                        class="w-full h-48 object-cover rounded-lg mb-5 shadow-md hover:shadow-lg transition-shadow duration-300">
                @else
                    <img src="{{ asset('images/default.jpg') }}" alt="Default Package Image"
                        class="w-full h-48 object-cover rounded-lg mb-5 shadow-md hover:shadow-lg transition-shadow duration-300">
                @endif
                ...
                @if (session('checkout_data.is_custom'))
                    ...
                    <p class="text-lg font-medium text-gray-700 mt-2 flex items-center">
                        ...
                        Package Type: <span
                            class="text-yellow-600 font-bold ml-1">{{ ucfirst(session('checkout_data.package_type')) }}</span>
                    </p>
                @endif

                <div class="mt-3">
                    @if (session('checkout_data.is_custom'))
                        <a href="{{ route('home') }}#custom-package"
                            class="inline-block w-full text-center py-2 bg-blue-500 text-white font-bold text-lg rounded-lg hover:bg-blue-600 transition-all">
                            Edit Custom Package
                        </a>
                    @else
                        <a href="{{ route('packages.show', $package->id ?? 1) }}"
                            class="inline-block w-full text-center py-2 bg-blue-500 text-white font-bold text-lg rounded-lg hover:bg-blue-600 transition-all">
                            Edit Information
                        </a>
                    @endif
            ...
            <!-- User Information -->
            <div
                class="col-span-1 bg-gradient-to-r from-white to-gray-100 p-6 rounded-lg shadow-lg transform hover:scale-105 transition-transform duration-300 ease-in-out">
            ...
            <div
                ...
                <div>
                    ...
                    <h2 class="text-2xl font-semibold text-gray-800 mb-5 flex items-center">
                        ...
                        Total: <span class="text-blue-500 ml-2">Rs. {{ number_format($total_price, 2) }}</span>
                    </h2>
                    ...
                    <div class="flex items-center mb-5">
                        ...
                        <select id="paymentMethod" name="payment_method"
                            class="w-full border rounded-lg p-4 shadow-sm bg-gray-100 text-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <div class="bg-gray-100 p-4 mt-1 rounded-lg mb-9 shadow-sm">
                        ...
                        <p class="text-gray-700 text-md">
                            ...
                            contact support at <a href="mailto:support@yatrasathi.com"
                                class="text-blue-500 underline">support@yatrasathi.com</a>.
                        </p>
                    </div>
                </div>
                ...
                <div>
                    <button id="paymentButton" type="button"
                        class="w-full py-2 rounded-lg text-white text-lg font-semibold shadow-lg transition duration-300 ease-in-out">
                        Confirm Book
                    </button>
                </div>
            </div>
    ...
    <form id="esewa" action="https://rc-epay.esewa.com.np/api/epay/main/v2/form" method="POST" class="hidden">
        ...
        <input type="hidden" id="success_url" name="success_url"
            value="{{ route('order.storeEsewa', [$package->id ?? 0, $num_people, $duration, $travel_date]) }}" required>
